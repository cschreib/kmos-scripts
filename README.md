...
Other calibration sets can contain various types of images, not always the same. Usually this is 5 "dark", 3 "flat-off", 18 "flat-lamp", 1 "wave-off" and 6 "wave-lamp". Often there are 10 "dark", but the pipeline can only handle 5, so I only keep the 5 latest and place the earliest 5 into a sub-directory "unused" (this is important else you can get errors in the pipeline). In addition, sometimes you have 4 "flat-sky" a bit before the calibration set (example for you at 2015-12-30T23:31:45.547, I put it into the 3rd calibration set).

If I do that for your data, I get 6 OBs, 4 calibration sets and 5 standard star calibration sets.
...
- the value of GRATING matches your observations (the format is XXX, where X is the name of the band in which you observe; in your case its the K band, so GRATING=KKK; in our own KMOS programs we used H+K, so GRATING=HKHKHK; you get the point).
...
note: no 'flat-sky' frame in /home/twang/data/kmos-cluster/calib-01/
...
However, "calib-01" doesn't include illumination correction. If you noted down the result of step (C.3), you will find that in fact only "calib-03" has it. So we add it to the list:
```bash
...
3. This will show the cube of the 1st IFU in the file. Unfortunately, for the standard stars, only 3 IFUs are actually used, so you may have to open a different extension to actually see something. For example, for "calib-std-01", you can try extension "5".
...
4. Now each OB has been reduced individually, with the sky subtracted. The pipeline can recognize automatically if you have put science targets in both the A and B configurations, so you don't have anything special to do. In a next step we will merge these OBs into a single thing, but for now it is important to make sure that the reduction was successful (no bug or crash). Just go into each "sci-XX" directory: there should be 4 FITS files, corresponding to each of the 4 dither positions you specified in your OBs. Each FITS file contains 48 extensions, two per IFU (one for the flux and the other for the uncertainty). You don't need to check them one by one (see next point), just make sure the files are there.
...
2. First you are going to need the names of your helper targets, as given in the KARMA file when you prepared the OBs. To find out the names of all your targets, go into, e.g., "sci-01" and run the following command on any of the 4 FITS images:
```bash
fitshdr sci_reconstructed_KMOS.2015-12-26T05\:56\:49.884-sci.fits | grep -E "ARM[0-9]+ NAME"
```

3. This will print the list of the IFUs "ARMx" (where "x" is the IFU ID) and the corresponding targets. From there, identify the name of your helper targets. In Tao's case, it seems you named your targets with the Muzzin ID, so it should be straightforward. I figured that, for the first 3 OBs, you have no stars and one nearby galaxy with ID=129994. I'm not sure there is anything for the last 3 OBs, please check that. If this is not true, could you tell me the IDs? I'll need that for more advanced reduction.
...
This will open DS9 and display the continuum images of all your helper targets in the OB, for each of the 4 dither positions. Offsets are normal and expected, for now just make sure that they are detected. For Tao's data it seems that the z=0 galaxy is detected in the first 3 OBs, but faint. The detection is clearer in the 3rd OB, and I suppose this is just due to good seeing.
